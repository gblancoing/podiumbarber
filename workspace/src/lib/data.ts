...
if (!globalForDb.bookingsDb) {
  globalForDb.bookingsDb = [];
}
...
export async function getBookings(): Promise<Booking[]> {
  ...
  return Promise.resolve([...globalForDb.bookingsDb]);
}
...
export async function saveNewBooking(bookingData: Omit<Booking, 'id' | 'status'>): Promise<Booking> {
  ...
  globalForDb.bookingsDb.push(newBooking);
  console.log('Cita guardada:', newBooking);
  console.log('Todas las citas:', globalForDb.bookingsDb);
...
export const getAvailableTimeSlots = (date: Date, stylistId: string) => {
  ...
  const existingBookings = globalForDb.bookingsDb.filter(b => b.stylistId === stylistId && b.date === dateString);
